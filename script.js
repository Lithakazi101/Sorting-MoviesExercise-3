...
window.onload = function() {
    ...
    let sortedAttr = sortMoviesByAttr(movies, 'rank');
    // Display Movies list
...
function sortMoviesByRank(movies){
  ...
  for (let j = 0; j < movies.length - 1; j++) {
      ...
      let movies_max = movies[j];
      let max_location = j;

      for (let i = j; i < movies.length; i++) {
          if (movies[i].rank > movies_max.rank) {
...
function sortMoviesByAttr(movies, sortAttr){
  ...
  for (let j = 0; j < movies.length - 1; j++) {
    ...
    // for (let i = j; i < movies.length; i++) {
    //     if (movies[i][sortAttr] > movies_max[sortAttr]) {
    //         // Know max AND it's index (location)
    //         movies_max = movies[i]
    //         max_location = i
    //     }
...
function getMaxMovieObject(movies, start, sortAttr){
  ...
  return {max_movies: movies_max, max_index: max_location}
}
